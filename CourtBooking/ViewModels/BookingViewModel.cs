...
public class CourtAvailabilityViewModel
{
    ...
    /// <summary>Subset of <see cref="PendingHours"/> whose booking already has a payment proof
    /// submitted — further along than a bare reservation with no proof yet, so availability grids
    /// can label it "Awaiting confirmation" instead of a plain "Pending".</summary>
    public List<int> AwaitingConfirmationHours { get; set; } = new();
